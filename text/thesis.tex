...
Virtual Data Optimizer (VDO) is a block layer virtualization service in Linux storage stack. VDO enables user to operate with greater logical volume than is physically available. This is achieved by using deduplication, compression and elimination of zero-blocks.
...
Updating index is costly, so VDO is trying to minimize the updates. That is a reason the index can contain references to blocks that are no longer present in data. Those blocks are called stale blocks.
...
Storage stack for testing purposes is always prepared by executing a seuqence of LVM commands. For the simplest tests, one volume group and one logical volume was used to be a block device for VDO layer.
...
The prewritting should fulfill these conditions:
\begin{itemize}
